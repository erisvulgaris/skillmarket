...
import { ArrowLeft, Users, Wallet, Package, Shield, AlertTriangle, Activity, TrendingUp, Eye, Snowflake, Flame, CheckCircle2, Ban } from 'lucide-react'
...
export function AdminView() {
  ...
  const [tab, setTab] = useState<'dashboard' | 'users' | 'wallets' | 'services' | 'audit'>('dashboard')
  ...
  const loadUsers = useCallback(async () => {
    setLoading(true)
    try {
      const d = await api.get<{ items: any[] }>('/api/admin/users?limit=30')
      setUsers(d.items)
  ...
  const loadWallets = useCallback(async () => {
    setLoading(true)
    try {
      const d = await api.get<{ items: any[] }>('/api/admin/wallets?limit=30')
      setWallets(d.items)
    } catch {} finally { setLoading(false) }
  }, [])
  ...
  const loadServices = useCallback(async () => {
    setLoading(true)
    try {
      const d = await api.get<{ items: any[] }>('/api/admin/services?limit=30')
      setServices(d.items)
    } catch {} finally { setLoading(false) }
  }, [])
  ...
  const loadAudit = useCallback(async () => {
    setLoading(true)
    try {
      const d = await api.get<{ items: any[] }>('/api/admin/audit?limit=50')
      setAudit(d.items)
    } catch {} finally { setLoading(false) }
  }, [])
  ...
  }, [tab, loadDashboard, loadUsers, loadWallets, loadServices, loadAudit])
